...
const SourceStatus = ({ source }: SourceStatusProps) => {
  ...
  const changeSourceClassificationStatus = (status: string) => {
    const source_status_taxonomy = taxonomyList.filter(
      (t: any) => t.name === "Grandma Campaign Source Classification",
    )[0];

    if (source_status_taxonomy) {
      const already_has_status =
        source.classifications.filter(
          (c: any) =>
            c.taxonomy_id === source_status_taxonomy.id &&
            c.classification === status,
        ).length > 0;
      if (already_has_status) {
        dispatch(
          showNotification(
            "Source already has this classification status",
            "error",
          ),
        );
        ...
        dispatch(
          ...
          if (response.status === "success") {
            ...
            dispatch(
              showNotification(
                `Failed to update source status: ${response.message}`,
                "error",
              ),
            );
            setClicked(false);
        ...
        dispatch(
          showNotification(
            "You just requested a change in source classification status. Please wait for the request to  be completed.",
            "warning",
          ),
        );
      }
      ...
      dispatch(
        showNotification(
          "Failed to update source status: no grandma taxonomy found",
          "error",
        ),
      );
      setClicked(false);
  ...
  const changeSourceObsStatus = (status: string) => {
    const source_obs_taxonomy = taxonomyList?.filter(
      (t: any) => t.name === "Grandma Campaign Source Observation",
    )[0];

    if (source_obs_taxonomy) {
      const already_has_obs =
        source.classifications.filter(
          (c: any) =>
            c.taxonomy_id === source_obs_taxonomy.id &&
            c.classification === status,
        ).length > 0;
      if (already_has_obs) {
        dispatch(
        ...
        dispatch(
          showNotification(
            "You just requested a change in source obs status. Please wait for the request to  be completed.",
            "warning",
          ),
        );
      }
